...
export default function NumberOfBeds({
    value,
    onChange,
}: NumberOfBedsProps){
    const decreaseBeds = ()=>{
    ...
    return(
        <div className="border-b border-[#e5e7eb] px-3 py-3">
           ...
           <div className="flex h-10 items-center justify-between rounded-md border-[#d9dce5] bg white px-3">
            <button type="button" onClick={decreaseBeds}
            aria-label="Decrease number of beds"
            className="text-xs leading-none hover:text-[#004f3e]"
            >
                <img src="/assets/decrease-icon.svg" alt="decrease" className="h-4 w-4" />
            </button>
        ...
        >
          <img src="/assets/increase-icon.svg" alt="increase" className="h-4 w-4" />
        </button>
           </div>
        </div>
    )
